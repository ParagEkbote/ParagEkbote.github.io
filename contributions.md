...
**Combined repository stars:** 471,690 ⭐
...
- [[CI] Set explicit uv version in gh-actions and send authenticated requests to reduce flakiness ](https://github.com/PrunaAI/pruna/pull/473) — `PrunaAI/pruna` _2026-01-15_
...
- `huggingface/transformers` → 6 PRs, ⭐ 161,075
- `pytorch/pytorch` → 2 PRs, ⭐ 100,275
- `hiyouga/LlamaFactory` → 1 PR, ⭐ 71,719
- `huggingface/diffusers` → 19 PRs, ⭐ 33,728
- `huggingface/trl` → 2 PRs, ⭐ 18,494
- `dagster-io/dagster` → 1 PR, ⭐ 15,596
- `optuna/optuna` → 7 PRs, ⭐ 14,268
- `Lightning-AI/litgpt` → 1 PR, ⭐ 13,389
- `linkedin/Liger-Kernel` → 11 PRs, ⭐ 6,394
...
- `huggingface/transformers` → ⭐ 161,075, forks 33,367, open issues 2,377
- `pytorch/pytorch` → ⭐ 100,275, forks 27,893, open issues 18,352
- `hiyouga/LlamaFactory` → ⭐ 71,719, forks 8,760, open issues 1,019
- `huggingface/diffusers` → ⭐ 33,728, forks 7,008, open issues 1,169
- `huggingface/trl` → ⭐ 18,494, forks 2,755, open issues 655
- `dagster-io/dagster` → ⭐ 15,596, forks 2,152, open issues 2,667
- `optuna/optuna` → ⭐ 14,268, forks 1,330, open issues 42
- `Lightning-AI/litgpt` → ⭐ 13,389, forks 1,446, open issues 258
- `linkedin/Liger-Kernel` → ⭐ 6,394, forks 536, open issues 142
- `skorch-dev/skorch` → ⭐ 6,163, forks 411, open issues 64
- `mlabonne/llm-datasets` → ⭐ 4,619, forks 383, open issues 11
- `huggingface/notebooks` → ⭐ 4,558, forks 1,817, open issues 225
- `huggingface/blog` → ⭐ 3,429, forks 1,018, open issues 244
- `argilla-io/distilabel` → ⭐ 3,232, forks 242, open issues 97
- `alibaba/ROLL` → ⭐ 3,184, forks 287, open issues 110
- `huggingface/cookbook` → ⭐ 2,668, forks 411, open issues 60
- `huggingface/lighteval` → ⭐ 2,429, forks 475, open issues 316
- `gradio-app/trackio` → ⭐ 1,493, forks 116, open issues 10
- `PrunaAI/pruna` → ⭐ 1,192, forks 89, open issues 65
...
- `optuna/optuna-examples` → ⭐ 851, forks 197, open issues 3
- `Meesho/BharatMLStack` → ⭐ 692, forks 77, open issues 32
- `cfahlgren1/observers` → ⭐ 253, forks 27, open issues 18
- `optuna/optuna-integration` → ⭐ 70, forks 43, open issues 11
- `dagster-io/community-integrations` → ⭐ 54, forks 44, open issues 31
